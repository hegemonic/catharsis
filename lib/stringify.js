...
Stringifier.prototype.type = function(type) {
	...
	switch(type.type) {
		case Types.AllLiteral:
			result += this._addModifiers(type, this._formatNameAndType(type, '*'));
			break;
		case Types.FunctionType:
			result += this._signature(type);
			break;
		case Types.NullLiteral:
			result += this._addModifiers(type, this._formatNameAndType(type, 'null'));
			break;
		case Types.RecordType:
			result += this._addModifiers(type, this._record(type));
			break;
		case Types.TypeApplication:
			result += this._addModifiers(type, this.type(type.expression));
			result += this.applications(type.applications);
			break;
		case Types.UndefinedLiteral:
			result += this._addModifiers(type, this._formatNameAndType(type, 'undefined'));
			break;
		case Types.TypeUnion:
			result += this._addModifiers(type, this.elements(type.elements));
			break;
...
// Adds nullable and repeatable modifiers if necessary.
...
Stringifier.prototype._addLinks = function(nameString) {
	...
	if (options.links && Object.prototype.hasOwnProperty.call(options.links, nameString)) {
		...
		openTag = '<a href="' + options.links[nameString] + '"' + cssClass + '>';
...
Stringifier.prototype._signature = function(type) {
	...
	result = this._addModifiers(type, signatureBase);
